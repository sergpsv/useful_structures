...
is
  ...
  begin
    if Recipients is not null and Recipients.count>0 then 
      ...
      l_sendres := -10;
      l_errmsg  := 'íåò ñïèñêà ïîëó÷àòåëåé';
    end if;

    ------------------------------------------------------------
    -- åñëè îòïðàâêà ïî ïî÷òå íå óäàëàñü
    if l_sendres < 0 then 
      ...
      if l_res =0 then 
        execute immediate 'create table not_sended_email
                         ...
                         pctfree 0 ';
        execute immediate 'comment on table not_sended_EMAIL is ''Âëîæåíèÿ íå îòïðàâëåííûå îáúåêòîì TMAIL ïî ïðè÷èíå err_msg''';
      end if;
      ...
      commit;
    end if; -- åñëè îòïðàâêà ïî ïî÷òå íå óäàëàñü
